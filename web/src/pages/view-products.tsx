...
export const ViewProduct = (p: RouteComponentProps<{ product: string }>) => {
    ...
    const { name, pret, discount, imageurl } = product;
    let percent = 0;
    if (discount) {
        percent = 100 - Math.round((100 * (pret - discount)) / pret)
    }
    return <>
            ...
            <Grid container>
                <Grid item xs={6}>
                    <div style={{ display: "flex", flexDirection: "column", marginLeft: "10px", marginBottom: "5px" }}>
                        <div title={product.discount ? `${product.pret} LEI` : ""} style={{ userSelect: "none", fontFamily: "proxima-nova", fontSize: "15px", color: "#f03", textDecoration: "line-through", minHeight: "14.4px", marginTop: "10px" }}>
                            {product.discount ? `${product.pret} LEI` : ""}
                        </div>
